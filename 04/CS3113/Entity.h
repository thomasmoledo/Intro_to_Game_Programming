...
enum Direction    { LEFT, UP, RIGHT, DOWN, IDLE_RIGHT, IDLE_LEFT, JUMP_RIGHT, JUMP_LEFT, FALL_RIGHT, FALL_LEFT };
enum EntityStatus { ACTIVE, INACTIVE                   };
enum EntityType   { PLAYER, BLOCK, PLATFORM, NPC, NONE };
enum AIType       { WANDERER, FOLLOWER, LERPER, FLYER, CHARGER };
...
private:
    ...
    void AIFly(Entity *target, float deltaTime);
    void AICharge(Entity *target);
    static Direction atlasDirection(Direction direction);
...
public:
    ...
    void resetMovement() {
        ...
        {
            if (mDirection == LEFT) mDirection = IDLE_LEFT;
    ...
    void setLerpFactor(float newLerpFactor)
        { mLerpFactor = newLerpFactor; }
    void resetCurrFrameIndex() { mCurrentFrameIndex = 0; }
};
#endif // ENTITY_H